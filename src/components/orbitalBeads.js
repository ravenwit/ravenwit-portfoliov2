...
export function initOrbitalBeads() {
    ...
    beadsData.forEach((data, i) => {
        ...
        // Position from cradle top-center. Arc center is (210, 60) so endpoints are near top, arc hangs down
        const beadSize = data.type === 'document' ? 52 : 42;
        bead.style.left = `${210 + pos.x - beadSize / 2}px`;
        bead.style.top = `${60 + pos.y - beadSize / 2}px`;
    ...
    cradle.addEventListener('mousemove', (e) => {
        ...
        // Arc center is at 210, 60 within the cradle
        const cx = rect.left + 210;
        const cy = rect.top + 60;
        mousePos.x = e.clientX - cx;
        mousePos.y = e.clientY - cy;
        mouseInCradle = true;
    });
    ...
    function tick() {
        ...
        const cx = 210;  // Arc center X in the cradle
        const cy = 60;    // Arc center Y in the cradle
        ...
        strings.forEach((s, idx) => {
            ...
            for (let j = 0; j <= NUM_STRING_POINTS; j++) {
                const t = j / NUM_STRING_POINTS;
                // Rest position (line between beads)
                const rx = s.p0.x + t * (s.p1.x - s.p0.x);
                const ry = s.p0.y + t * (s.p1.y - s.p0.y);
                // Add catenary sag + displacement
                const sag = Math.sin(t * Math.PI) * 8; // slight sag
                const disp = pts[j].displacement;
                const drawX = cx + rx + s.nx * (disp + sag);
                const drawY = cy + ry + s.ny * (disp + sag);

                if (j === 0) sctx.moveTo(drawX, drawY);
                else sctx.lineTo(drawX, drawY);
            }
            ...
            const glowAlpha = Math.min(1, 0.3 + energy * 0.05);
            const isDoc = beadsData[idx].type === 'document' || beadsData[idx + 1].type === 'document';
            const color = isDoc ? `rgba(192, 132, 252, ${glowAlpha})` : `rgba(0, 255, 255, ${glowAlpha})`;

            sctx.strokeStyle = color;
            sctx.lineWidth = 1.5;
            sctx.shadowBlur = energy > 1 ? Math.min(15, energy * 2) : 0;
            sctx.shadowColor = isDoc ? '#c084fc' : '#0ff';
            sctx.stroke();
            sctx.shadowBlur = 0;
        });
    }
